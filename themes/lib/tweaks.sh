...
# Vague GTK Theme: SASS Tweaks Manager
...
# Applies the Medium background color scheme.
medium_color() {
    run_safe sed -i "/@import/s/color-palette-default/color-palette-medium/" "${SRC_DIR}/sass/_tweaks-temp.scss"
    run_safe sed -i "/\$colorscheme:/s/default/medium/" "${SRC_DIR}/sass/_tweaks-temp.scss"
}

# Applies the Soft background color scheme.
soft_color() {
    run_safe sed -i "/@import/s/color-palette-default/color-palette-soft/" "${SRC_DIR}/sass/_tweaks-temp.scss"
    run_safe sed -i "/\$colorscheme:/s/default/soft/" "${SRC_DIR}/sass/_tweaks-temp.scss"
}
...
apply_theme_tweaks() {
    ...
    if [[ "${DRY_RUN:-}" != "true" ]]; then
        ...
        if [[ "$medium"     == "true" ]]; then medium_color;     fi
        if [[ "$soft"       == "true" ]]; then soft_color;       fi
...
report_tweaks() {
    ...
    # Background scheme
    if   [[ "$blackness" == "true" ]]; then printf "\r\033[K│ %-20s : '%s'\n" "Theme base color" "Blackness"; count=$((count + 1))
    elif [[ "$medium"    == "true" ]]; then printf "\r\033[K│ %-20s : '%s'\n" "Theme base color" "Medium"; count=$((count + 1))
    elif [[ "$soft"      == "true" ]]; then printf "\r\033[K│ %-20s : '%s'\n" "Theme base color" "Soft"; count=$((count + 1))
