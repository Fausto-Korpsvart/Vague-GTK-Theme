...
_THEME_VARIANTS=('' '-Blue' '-Green' '-Grey' '-Orange' '-Pink' '-Purple' '-Red' '-Teal' '-Yellow')
...
BATCH_MODE=true "$THEMES_ROOT_DIR/install.sh" -d "$THEME_DIR" --tweaks medium -a all
BATCH_MODE=true "$THEMES_ROOT_DIR/install.sh" -d "$THEME_DIR" --tweaks soft -a all
